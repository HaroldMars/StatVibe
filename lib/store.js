...
//   1. MongoDB when MONGO_URI / MONGODB_URI is set (preferred durable store)
//   2. Vercel KV / Upstash Redis when KV_REST_API_* or UPSTASH_REDIS_REST_* is set
//   3. Cloudinary raw asset when CLOUDINARY_* is set on Vercel (durable fallback)
//   4. Local JSON file otherwise — ./data locally, /tmp on serverless (EPHEMERAL)
...
const MONGO_DOC_ID = 'statvibe_db';

const KV_URL = process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_REST_URL;
const KV_TOKEN = process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN;
const useKV = !useMongo && !!(KV_URL && KV_TOKEN);
...
const useCloudinary = !useMongo && !useKV && !!(CLOUD_NAME && CLOUD_KEY && CLOUD_SECRET) && (onServerless || forceCloudStore);
...
function backendInfo() {
  if (useMongo) return { kind: 'mongodb', durable: true, docId: MONGO_DOC_ID };
...
async function mongoCollection() {
  ...
  if (mongoColPromise) return mongoColPromise;
  mongoColPromise = (async () => {
    ...
    // DB name comes from the URI path (e.g. ...mongodb.net/MIXED).
    return mongoClient.db().collection('statvibe_store');
  })();
  return mongoColPromise;
}
...
async function mongoLoad() {
  ...
  if (!doc) return null;
  const { _id, updatedAt, ...payload } = doc;
  return payload;
}

async function mongoSave(payload) {
  const col = await mongoCollection();
  await col.updateOne(
    { _id: MONGO_DOC_ID },
...
const ready = (async () => {
  // Skip during `vite build` / static asset builds so Vercel production env
  // (MONGO_URI, Cloudinary, …) cannot hang or fail the frontend build step.
  const lifecycle = process.env.npm_lifecycle_event || '';
  if (lifecycle === 'build' || process.env.STATVIBE_SKIP_STORE_LOAD === '1') {
    console.log('[store] skip eager load during build (backend=' + backendInfo().kind + ')');
    return;
  }
  try {
    if (useMongo) {
      const v = await mongoLoad();
      if (v) { db = { ...fresh(), ...v }; rebuildIndexes(db); }
    } else if (useKV) {
      const v = await kvCmd(['GET', KV_KEY]);
  ...
  saving = (async () => {
    do {
      ...
      try {
        ...
        if (useMongo || useKV || useCloudinary || onServerless) {
          try {
            const remote = await loadRemoteDb();
...
module.exports = new Proxy(store, {
  get(target, prop) {
    ...
    return async (...args) => withLock(async () => {
      ...
      if (useMongo || useKV || useCloudinary) await reloadFromBackend();
