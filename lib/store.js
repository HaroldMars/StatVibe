...
let saving = null, saveAgain = false, lastPersistError = null;
function persist() {
...
/** Serialize store ops so a reload cannot wipe another in-flight register/login write. */
...
// backend. Ops are serialized so a concurrent reload cannot wipe an in-flight
// register/login before it is persisted (root cause of "account not found"
// after a successful signup on serverless).
module.exports = new Proxy(store, {
  get(target, prop) {
    const v = target[prop];
    if (typeof v !== 'function') return v;
    if (prop === 'backend' || prop === 'reload' || prop === '_reset') {
      return async (...args) => enqueue(async () => {
        await ready;
    ...
    return async (...args) => enqueue(async () => {
      await ready;
      // Durable backends: always refresh before read/write so auth/inventory stay in sync.
