...
//   • Vercel KV (Upstash Redis) when KV_REST_API_URL is set — durable, no npm
//     package (REST via fetch). Use this in production on Vercel.
//   • Local JSON file otherwise — ./data locally, /tmp on serverless (writable
//     but EPHEMERAL, so KV is strongly recommended on Vercel). See ROADMAP.
// For heavy production scale, add a Postgres adapter with the same methods.
...
const EMPTY = { users: {}, byEmail: {}, byPhone: {}, byTag: {}, sessions: {}, accounts: {}, inventory: {}, ideas: {}, aiHistory: {}, conversations: {}, messages: {}, admins: {}, adminSessions: {} };
...
const ready = (async () => {
  if (useKV) {
    try { const v = await kvCmd(['GET', KV_KEY]); if (v) db = { ...fresh(), ...JSON.parse(v) }; }
    catch (e) { console.error('[store] KV load failed:', e.message); }
  } else {
...
// Proxy so every method awaits the initial load before touching `db`.
