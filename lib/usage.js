...
function canConsume(account, n = 1) {
  ...
  // Allow the request to start if any tokens remain (last reply may slightly overshoot).
  if (a.aiLimit - a.aiUsed <= 0) {
    return { ok: false, account: a, code: 'quota_exceeded', error: 'AI token limit reached. Upgrade your plan.' };
  }
  return { ok: true, account: a, need };
}

function consume(account, n = 1) {
  const check = canConsume(account, 1);
  if (!check.ok) return check;
  return billTokens(check.account, n);
}

/** Always apply token usage after a successful AI reply (pre-checked via canConsume). */
function billTokens(account, n = 1) {
  const a = ensureUsage(account);
  if (a.plan === 'Enterprise') return { ok: true, account: a, usage: usageView(a) };
...
module.exports = {
  ...
  periodMsFor, periodLabel, ensureUsage, usageView, canConsume, consume, billTokens,
  estimateTokens, countMessageTokens,
};
