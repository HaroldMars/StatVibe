...
function resolveExistingDir(candidates, probeFile) {
  for (const c of candidates) {
    if (fs.existsSync(probeFile ? path.join(c, probeFile) : c)) return c;
  }
  return candidates[0];
}
// Monorepo: prefer local public/, then sibling client package, then repo-root public/.
const PUBLIC_DIR = resolveExistingDir(
  [
    path.join(__dirname, 'public'),
    path.join(__dirname, '../statvibe-client/public'),
    path.join(__dirname, '../public'),
  ],
  'index.html'
);
const DIST_DIR = resolveExistingDir(
  [
    path.join(__dirname, 'dist'),
    path.join(__dirname, '../statvibe-client/dist'),
    path.join(__dirname, '../dist'),
  ],
  'index.html'
);
...
async function handleModels(res) {
  ...
  const hostedEngines = hostedConfigured()
  ...
  const engines = ollamaModels.length ? ollamaModels : hostedEngines;
  // Always expose clean Gemini catalog entries for the workspace selector.
  const workspace = WORKSPACE_MODELS.map((m) => ({
    ...m,
    kind: 'workspace',
    available: hostedConfigured() || ollamaModels.length > 0,
  }));
