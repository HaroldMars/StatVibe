...
/**
 * Absolute API origin when the client is hosted separately from the API
 * (e.g. NEXT_PUBLIC_API_URL=https://statvibe-server.vercel.app). Empty = same origin.
 */
export function apiOrigin() {
  try {
    if (typeof import.meta !== 'undefined' && import.meta.env) {
      const v = import.meta.env.VITE_API_URL || import.meta.env.NEXT_PUBLIC_API_URL || '';
...
/** Build a full /api URL. `path` is like `/auth/me` or already `/api/...`. */
...
export async function api(path, { method = 'GET', body, auth = true } = {}) {
  ...
  let r;
  try { r = await fetch(apiUrl(path), { method, headers, body: body ? JSON.stringify(body) : undefined }); }
